...
- (void)viewDidLoad {
    ...
    if (!_demandTextView) {
        _demandTextView = [[UITextView alloc] initWithFrame:CGRectMake(0, 74, SCREEN_WIDTH, 150)];
...
- (void)rightItemClick
{
    if (_demandTextView.text.length == 0) {
        [[iToast makeText:@"请输入您的宝贵意见!"] show];
        return;
    }
    ...
    NSArray *dataArray = [[NSArray alloc] initWithObjects:userInfoModel.userId,_demandTextView.text,@"2.0",@"IOS", nil];
    NSString *hamcString = [[communcation sharedInstance] ArrayCompareAndHMac:dataArray];
    
    In_FeedbackModel *inModel = [[In_FeedbackModel alloc] init];
    inModel.key = userInfoModel.userId;
    inModel.user_id = userInfoModel.userId;
    inModel.digest = hamcString;
    inModel.text = _demandTextView.text;
    inModel.version = @"2.0";
    inModel.system = @"IOS";
    
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        Out_FeedbackModel *outModel = [[communcation sharedInstance] userFeedBackWithModel:inModel];
