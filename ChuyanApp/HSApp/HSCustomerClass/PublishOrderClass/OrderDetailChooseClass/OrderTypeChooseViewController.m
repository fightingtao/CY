//
//  OrderTypeChooseViewController.m
//  HSApp
//
...
@interface OrderTypeChooseViewController ()
...
@implementation OrderTypeChooseViewController
@synthesize delegate;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (!_typeArray || _typeArray.count == 0) {
        [self getTypeData];
    }
}
...
- (void)getTypeData
{
    ...
    if (userInfoModel&&userInfoModel.userId&&![userInfoModel.userId isEqualToString:@""])
    {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            Out_TypeModel *outModel = [[communcation sharedInstance] getTypesWithKey:userInfoModel.key AndDigest:hmacString];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    OutTypeBody *model = [_typeArray objectAtIndex:indexPath.section];
    contentLabel.text = model.type_name;
    return cell;
