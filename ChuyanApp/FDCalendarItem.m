...
@interface FDCalendarItem () <UICollectionViewDataSource, UICollectionViewDelegate,UIAlertViewDelegate>
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //可以把通过这种方法只获取年月日 ，而没有时分秒和时区
    NSDateFormatter *formatter1 = [[NSDateFormatter alloc] init];
    [formatter1 setDateFormat:@"yyyy-MM-dd"];
     NSTimeInterval ShiJianChuo1 = [selectedDate timeIntervalSince1970];
     DateSt = [formatter1 stringFromDate:selectedDate];

    NSDate *NowDate = [NSDate date];
    NSDateFormatter *formatter2 = [[NSDateFormatter alloc]init];
    [formatter2 setDateFormat:@"yyyy-MM-dd"];
    NSTimeInterval ShiJianChuo2 = [NowDate timeIntervalSince1970];
//    NSString *NowTime = [formatter2 stringFromDate:NowDate];

    NSString *MessageStr = nil;
    if (ShiJianChuo1<ShiJianChuo2) {
        MessageStr = @"只能选当天以后的时间";
        [[iToast makeText:MessageStr] show];
        return;
    }
    else if (ShiJianChuo1 == ShiJianChuo2)
    {
        MessageStr = @"只能选当天以后的时间";
        [[iToast makeText:MessageStr] show];
        return;
    ...
    {
        MessageStr = DateSt;
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"下次配送时间" message:MessageStr delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
        [alert show];
    }
...
{
    if (buttonIndex == 1){
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SelectDate" object:nil userInfo:@{@"date":DateSt}];
    }
